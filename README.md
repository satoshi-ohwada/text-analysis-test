...
5. [多彩な分析・可視化機能](#5-多彩な分析可視化機能)
   ...
   - [UMAP散布図（多様体学習・単語クラスタ）](#5-6-umap散布図多様体学習単語クラスタ)
   - [トピック分析（LDAモデル）](#5-7-トピック分析ldaモデル)
...
```mermaid
graph LR
    step1["1. テキスト入力<br/>（ファイル読込 / 貼付）"] --> step2["2. フィルター設定<br/>（品詞・頻出度の絞込）"]
    step2 --> step3["3. 分析・可視化<br/>（ワードクラウド・共起・UMAP等）"]
```

| ステップ | 操作内容 | 主なポイント |
| :---: | :--- | :--- |
| **Step 1** | **テキスト入力** | テキストファイル（`.txt` / `.csv` / `.tsv`）のドロップ、または直接貼り付け |
| **Step 2** | **フィルター設定** | 対象品詞（名詞・動詞等）の選択、最小出現回数の調整でノイズ除去 |
| **Step 3** | **分析・可視化** | プルダウンから分析手法（ワードクラウド、共起、PCA、UMAP、LDA）を選んで可視化 |
...
* 近くに配置された単語は「似た文脈で使われやすい」ことを示しており、テキスト内の「全体的な傾向や方向性」を大局的に俯瞰できます。

### 5-6. UMAP散布図（多様体学習・単語クラスタ）
* 最新の非線形次元圧縮アルゴリズム (UMAP) により、文脈が緊密な単語群をギュッと凝縮させた「意味の島（クラスタ）」として可視化します。
* PCAでは原点付近に集まりがちだった単語群が、UMAPではテーマごとに鮮明に島状に分離されるため、直感的にどのような話題のグループがあるかを明確に発見できます。
